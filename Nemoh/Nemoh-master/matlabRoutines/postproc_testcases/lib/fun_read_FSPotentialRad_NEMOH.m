function [w,k,Npanel,Nwline,XX,POT]=fun_read_FSPotentialRad_NEMOH(wd,FileN)
...
POT=zeros((2^Isym)*NPFlow,2,Nw,Ndof);
w=zeros(Nw,1);k=w;
...
for iw=1:Nw
    for irad=1:Ndof
        ...
        POT(1:NPFlow,1,iw,irad)=ligne(4:3+NPFlow);
        POT(1:NPFlow,2,iw,irad)=ligne(4+NPFlow:3+2*NPFlow);
        if Isym==1
            POT(NPFlow+1:2*NPFlow,1,iw,irad)=ligne(4+2*NPFlow:3+3*NPFlow);
            POT(NPFlow+1:2*NPFlow,2,iw,irad)=ligne(4+3*NPFlow:3+4*NPFlow);
        end
